...
function filterSprite($argv) {
    ...
    try {
        ...
        for ($i = 1; $i < count($argv); $i++) {
            ...
            // if (strpos($arg, "--output") === 0 || strpos($arg, "--columns") === 0 || strpos($arg, "--padding") === 0 || strpos($arg, "--override") === 0) {
            //     $output_param = explode("=", $arg, 2);
            //     $param = $output_param[0];
            //     $param_value = $output_param[1];
            //     switch ($param) {
            //         case "--output-image":
            //             if (!is_numeric($param_value) || $param_value <= 0 || strpos($param_value, "-") === 0) {
            //                 throw new Exception("$param : utilisation incorrecte. Veuillez spécifier un nom de fichier sprite.");
            //             }
            //             $sprite_name = $param_value;
            //             break;
            //         case "--output-style":
            //             if (!is_numeric($param_value) || $param_value <= 0 || strpos($param_value, "-") === 0) {
            //                 throw new Exception("$param : utilisation incorrecte. Veuillez spécifier un nom de fichier CSS.");
            //             }
            //             $css_name = $param_value;
            //             break;
            //         case "--output-html":
            //             if (!is_numeric($param_value) || $param_value <= 0 || strpos($param_value, "-") === 0) {
            //                 throw new Exception("$param : utilisation incorrecte. Veuillez spécifier un nom de fichier HTML.");
            //             }
            //             $html_name = $param_value;
            //             break;
            //         case "--padding":
            //             if (!ctype_digit($param_value)) {
            //                 throw new Exception("Veuillez spécifier une taille de padding correcte. Par exemple \"10\" pour 10 pixels de padding.");
            //             }
            //             $padding = $param_value;
            ...
            //             break;
            //         case "--override-size":
            //             if (!ctype_digit($param_value) || $param_value <= 1) {
            //                 throw new Exception("Vous devez utiliser un chiffre pour forcer la taille des images au format SIZExSIZE. Celui-ci doit être d'au moins 1.");
            //             }
            //             $img_size = $param_value;
            //             break;
            //         case "--columns-number":
            //             if (!ctype_digit($param_value) || $param_value < 1) {
            //                 throw new Exception("Vous devez utiliser des chiffres pour valider le nombre de colonnes. Celui-ci doit être d'au moins 1.");
            //             }
            //             $nb_cols = $param_value;
            //             if (empty($param_value)) {
            //                 $nb_cols = 0;
            ...
            //         default:
            //             if (strpos($param, "-") === 0) {
            //                 throw new Exception("$arg : paramètre non reconnu par le programme.");
            //             } else {
            //                 $folder = $arg;
            //             }
            //             break;
            ...
            //     $i++;
            // } else {
            //     throw new Exception("$param : paramètre non reconnu par le programme.");
            // }
        }        
